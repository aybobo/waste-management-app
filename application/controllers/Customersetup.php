...
class Customersetup extends CI_Controller {
	...
	public function sectorhome()
	{
		if ($this->session->userdata('role') != 'Admin') {
			...
			$this->load->view('sector/sectorpage_view', ['areas' => $areas]);
	...
	public function editsector()
	{
		if ($this->session->userdata('role') != 'Admin') {
			...
			$this->load->view('inc/header_view');
			$this->load->view('inc/sidebar_view');
			$this->load->view('sector/editsector_view', ['sector' => $sector, 'areas' => $areas]);
			$this->load->view('inc/footer_view');
		}
	...
	public function addstreet()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($chkstr) {
				...
				//split the words in the street name
				$parts = explode(" ", $data['street']);

				//get the 1st character in the 1st word
				$firstWord = str_split($parts[0]);

				$data['streetcode'] = strtoupper($firstWord[0]);

				$chars = str_split($parts[1]);
				$j = 1;
				foreach ($chars as $char) {
					$char = strtoupper($char);
					$temp = $data['streetcode'] . $char;

					//check if $temp code is available
					$tempcode = $this->customersetup_model->checkstreetcode($temp);

					//code not in use, assign it
					if (!$tempcode) {
						$data['streetcode'] = $temp;
						break;
					}
					...
					if ($j == count($chars) ) {
						...
						for ($i=0; $i < 1000; $i++) { 
							...
							//check if $temp code is available
							$tmpcode = $this->customersetup_model->checkstreetcode($temp);
				...
				if ($updstr) {
					...
					if (!$customers) {
						...
						foreach ($results as $result) {
							...
							$code = 'R/' . $data['streetcode'] . '/' . $parts[2];
	...
	public function fetchactivesector() {
		if ($this->input->post('areaid')) {
			echo $this->customersetup_model->fetchactivesector($this->input->post('areaid'));
		}
	}
	...
	public function customerhome()
	{
		if ($this->session->userdata('role') != 'Admin') {
			...
			//get active sectors
			$activecommercialtypes = $this->customersetup_model->getactivecommercialtypes();
			if (!$activecommercialtypes) {
				$this->session->set_flashdata('msg', 'Create commercial types');
				return redirect('customersetup/commercialhome');
			}
			else {
				$types = $activecommercialtypes['rows'];
			}

			//get active sector area
			$activeareas = $this->customersetup_model->getactiveareas();
			$areas = $activeareas['rows'];

			$this->load->view('inc/header_view');
			$this->load->view('inc/sidebar_view');
			$this->load->view('customer/customerpage_view', ['cats' => $cats, 'areas' => $areas,
															'types' => $types]);
	...
	public function addcustomer()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($data['customertype'] == 'Commercial' && $data['commtype'] == '') {
				$this->session->set_flashdata('msg', 'Commercial type field is required');
			...
			if ($data['customertype'] == 'Residential') {
				...
				//get commercial type name
				$data['commtypename'] = $this->customersetup_model->getcommercialtypename($data);
			}
			...
			if ($data['customertype'] == 'Commercial') {
				...
				//get max commercial counter value
				$maxVal = $this->customersetup_model->fetchMaxCommercialVal();
				$num = $maxVal + 1;
				$data['customercode'] = 'CM/' . $data['commtype'] . '/' . $num;
				$data['counter'] = $num;
				...
				//get max counter value for street
				$val = $this->customersetup_model->fetchMaxResidential($streetid);
				$j = $val + 1;
				$data['customercode'] = 'R/' . $streetcode . '/' . $j;
				$data['counter'] = $j;
			}
	...
	public function editcustomer()
	{
		if ($this->session->userdata('role') != 'Admin') {
			...
			$activeareas = $this->customersetup_model->getotheractiveareas($areaid);
			$areas = $activeareas['rows'];
			...
			//get other active commercial types
			$typeId = $customer->commercialtype;
			$alltypes = $this->customersetup_model->getothercomercialtype($typeId);
			$types = $alltypes['rows'];
	...
	public function updatecustomer()
	{
		...
		$this->form_validation->set_rules('telno', 'Phone Number',
			'trim|is_natural|exact_length[11]');
		$this->form_validation->set_rules('area', 'Sector area', 'required');
		$this->form_validation->set_rules('customertype', 'Customer type', 'required');
		$this->form_validation->set_rules('sector', 'Sector', 'required');
		$this->form_validation->set_rules('street', 'Street', 'required');
		$this->form_validation->set_rules('debt', 'Debt', 'is_natural');
		...
		if ($this->form_validation->run()) {
			...
			if ($data['customertype'] == 'Commercial' && $data['commtype'] == '') {
				$this->session->set_flashdata('msg', 'Commercial type is required');
			...
			if (($data['sector'] == $data['oldsec']) && ($data['street'] == $data['oldstr'])
				&& ($data['houseno'] == $data['oldhouse']) && ($data['housetype'] == $data['oldcat'])) {
				$data['customercode'] = $data['oldcustomercode'];
			}
			else {
			...
			$data['customercode'] = $data['oldcustomercode'];
			$data['counter'] = $data['oldcounter'];

			$parts = explode("/", $data['oldcustomercode']);

			if (($data['oldcustomercode'] == '') || ($data['commtype'] != $data['oldcommtype'])
				|| (($data['customertype'] == 'Commercial' && count($parts) != 3))
				|| ($data['oldcustomertype'] != $data['customertype'])
				|| (($data['oldcustomertype'] == 'Residential' && $data['customertype'] == 'Residential') && ($data['oldstr'] != $data['street']))) {

				if ($data['customertype'] == 'Commercial') {
					if (count($parts) == 2) {
						$data['customercode'] = $parts[0] . '/' . $data['commtype'] . '/' . $parts[1];
			...
			if ($chkcustomer) {
				if ($data['oldno'] != $data['telno']) {
					...
					if ($updcustomer) {
						...
						if (($data['debt'] != $data['olddebt'])
							|| ($data['wallet'] != $data['oldwallet'])) {
							...
							$this->email->cc('hayorfad@gmail.com');
							...
							$message .= "Customer code:\t" . $data['customercode'] . "<br>";
	...
	public function savepayment()
	{
		...
		/*if ($data['paymode'] == 'Direct') {*/
			...
			if ($customer->customertype == 'Residential') {
				...
				if ($data['amount'] > $data['mntcharge']) { //payment for more than 1 month
					if ($data['mntcharge'] < 10000) { //monthly charge < 10000
						$agentcomm = 0;
					}
					elseif ($data['mntcharge'] == 10000) { //monthly charge = 10000
						$jj = $data['amount']/$data['mntcharge'];
						$agentcomm = $data['mntcharge'] * 0.25 * $jj;
					}
					else {
						$x = $data['amount'] / $data['mntcharge']; //no of monthly payment
						$part = ($data['mntcharge'] - 10000) * $x * 0.25;

						$y = $data['amount'] % $data['mntcharge']; //remainder
						$part1 = '';
						...
						$agentcomm = $part + $part1;
					}
				}
				elseif ($data['amount'] == $data['mntcharge']) {
					if ($data['mntcharge'] < 10000) { //monthly charge < 10000
						$agentcomm = 0;
					}
					elseif ($data['mntcharge'] == 10000) { //monthly charge = 10000
						$agentcomm = $data['mntcharge'] * 0.25;
					}
					else {
					...
					if ($data['amount'] < 10000) { //amt < 10000
						$agentcomm = 0;
					}
					elseif ($data['amount'] == 10000) { //amt = 10000
						$agentcomm = $data['amount'] * 0.25;
					}
					else {
						$diff = $data['amount'] - 10000;
						$agentcomm = $diff * 0.25;
					}
				}
			}
		...
		if ($resultid == 0) {
			$this->session->set_flashdata('msg', 'Error saving payment. Try again.');
			return redirect('home/index');
		}
		else {
			//save debit to cashbook
			$data['narration'] = $data['code'] . ', ' . $data['desc'];
			$debitCashBook = $this->customersetup_model->saveDebit($data);

			if ($data['lawmacomm'] > 0) {
				$data['details'] = 'Lagos State commission for payment made by ' . $data['code'];
				$creditCashBook = $this->customersetup_model->saveLawmaComm($data);
			}

			if ($data['agentcomm'] > 0) {
				$data['details'] = 'Agent commission for ' . $data['agentname'] . ' for payment made by ' . $data['code'];
				$creditCashBook = $this->customersetup_model->saveAgentComm($data);
			}
	...
	public function generateinvoice()
	{
		...
		if (($data['all'] == '') && ($data['sector'] == '') && ($data['street'] == '') && ($data['houseno'] == '') && !isset($data['id'])) {
			...
			if (!$search) {
				...
				foreach ($customers as $row) {
					...
					if ($invoiceyr != '0000') { // invoice has been generated for customer before
						if ($currentyr == $invoiceyr) {//invoice has been generated in current year
							if ($data['month'] > (int)$invoicemnt) {

								if ((int)$currentday >= 10) { //calculate invoice for current month too
									$outstandingmnt = $data['month'] - (int)$invoicemnt;
									$topay = ($row->monthlycharge + $vat) * $outstandingmnt;
									//$topay = $topay + $vat;
								...
								else { // do not calculate invoice for current month

									$outstandingmnt = (int)$data['month'] - ((int)$invoicemnt + 1);
									$topay = ($row->monthlycharge + $vat) * $outstandingmnt;
									//$topay = $topay + $vat;
							...
							if ($data['month'] == (int)$invoicemnt) { //invoice already generated in mnt
								if (((int)$currentday >= 10) && ((int)$invoiceday < 10)) {

									$topay = $row->monthlycharge + $vat;
									//$topay = $topay + $vat;
						...
						//
							...
							$x = 12 - (int)$invoicemnt;
							$y = (int)$currentyr - ((int)$invoiceyr + 1);

							$outstandingmnt = '';

							if ((int)$currentday >= 10) {
								$outstandingmnt = ($y * 12) + $x + (int)$data['month'];
							}
							else {
								$outstandingmnt = ($y * 12) + $x + ((int)$data['month'] - 1);
							}
						...
						if ($currentyr == $yearcreated) { //same year

							if ((int)$currentday >= 10) {
								//$outstandingmnt = (int)$data['month'] - (int)$mntcreated;
								$outstandingmnt = 1;
							}
							else {
								$outstandingmnt = (int)$data['month'] - ((int)$mntcreated + 1);
							}

							$topay = $outstandingmnt * ($row->monthlycharge + $vat);
							//$topay = $topay + $vat;
							...
							//arears in year customer moved in
							$mntarear = 12 -(int)$mntcreated;
							$yrarea = (int)$currentyr - ((int)$yearcreated + 1);
							$yrarea = $yrarea * 12;
							$yrarea += $mntarear;

							if ((int)$currentday >= 10) {
								$outstandingmnt = (int)$data['month'] + $yrarea;
							}
							else {
								$outstandingmnt = (int)$data['month'] + ((int)$yrarea + 1);
							}
					...
					$interval = date_interval_create_from_date_string('14 days');
					...
					if ($chkinvoice) {
						if ($chkinvoice->invoiceyear == $data['year']) { //same yr update only mnt
							if ($chkinvoice->invoicemonth != $data['monthx']) {
								...
								$split = explode("-", $chkinvoice->invoicegeneratedate);
								$lastday = $split[0];

								if (((int)$lastday < 20) && ((int)$currentday >= 20)) {
								$account = $this->customersetup_model->updatecustomeraccount($data);
								$addinvoice = $this->customersetup_model->addnewinvoice1($data);
								}
							}
				...
				//select payment for the month
				$paymentinmonth = $this->customersetup_model->paymentInMonth($data);
				$payments = $paymentinmonth['rows'];
				$count = $paymentinmonth['num'];
				$this->load->view('inc/header_view1');
				//$this->load->view('inc/sidebar_view');
				$this->load->view('invoice/printinvoice_view', ['rows' => $records, 'num' => $num, 'payments' => $payments, 'count' => $count]);
				$this->load->view('inc/footer_view');
			}
		}
	}
	...
	public function printreceipt()
	{
		...
		//get user signature using ID
		$signature = '';
		$usrSignature = $this->customersetup_model->getSignature($id);

		if (!$usrSignature) {
			$this->session->set_flashdata('msg', 'Kindly upload your signature');
		...
		$this->load->view('inc/header_view1');
		//$this->load->view('inc/sidebar_view');
		$this->load->view('payments/printreceipt_view', ['data' => $data, 'signature' => $signature]);
		$this->load->view('inc/footer_view');
	}
	...
	public function commercialhome()
	{
		if ($this->session->userdata('role') != 'Admin') {
			...
			$this->load->view('inc/header_view');
			$this->load->view('inc/sidebar_view');
			$this->load->view('commercial/commercialpage_view');
			$this->load->view('inc/footer_view');
		}
	}
	...
	public function addcommercialtype()
	{
		//set validation rules
		$this->form_validation->set_rules('commercial', 'Commercial type name', 'trim|required');

		if ($this->form_validation->run()) {
			$commercialname = $this->input->post('commercial');

			//check if areaname exists
			$chkcomm = $this->customersetup_model->checkCommercialType($commercialname);

			if ($chkcomm) {
				$this->session->set_flashdata('msg', 'Commercial type already exists');
				return redirect('customersetup/commercialhome');
			}
			else {
				$addcomm = $this->customersetup_model->addNewCommercialType($commercialname);

				if ($addcomm) {
					$this->session->set_flashdata('success', 'Commercial type added');
					return redirect('customersetup/commercialhome');
				}
			}
		}
	...
	public function loadcode()
	{
		...
		foreach ($customers as $customer) {
			...
			if ($customer->customertype == 'Commercial') {
				...
				$upd = $this->customersetup_model->updatecode($customerid, $customercode, $counter);
			}
			else {
				$streetid = $customer->streetid;
				$street = $this->customersetup_model->getsinglestreet($streetid);
				...
				$val = $this->customersetup_model->fetchMaxResidential($streetid);
				$j = $val + 1;
				$customercode = 'R/' . $streetcode . '/' . $j;
				$counter = $j;

				$upd = $this->customersetup_model->updaterescode($customerid, $streetid, $counter, $customercode);
			}

		}

		$this->session->set_flashdata('success', 'Yaaay');
		return redirect('customersetup/sectorhome');
	}

	//---------------------------------------

	public function streetcode()
	{
		//select all customers from db
		$allstreets = $this->customersetup_model->getallstreet();
		...
		foreach ($streets as $street) {
			...
			$streetcode = strtoupper($firstWord[0]);

			$chars = str_split($parts[1]);
			$j = 1;
			foreach ($chars as $char) {
				$char = strtoupper($char);
				$temp = $streetcode . $char;
			...
			$code = $this->customersetup_model->updatestrtcode($streetid, $streetcode);
			
			//update code here
		} //outer foreach
		$this->session->set_flashdata('success', 'Hurray!');
		return redirect('customersetup/sectorhome');
	}
	...
	{
		//select all invoice
		$allinvoices = $this->customersetup_model->getinv();
		...
		foreach ($invoices as $invoice) {
			...
			//update invoice
			$customer = $this->customersetup_model->updcustomerinvoice($customerid, $address);
		}
	...
	public function addinvoicebycode()
	{
		
		//load view
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('invoice/addinvoicebycode_view');
		$this->load->view('inc2/footer_view2');
	}

	//-------------------------------------------

	public function getinvoicebycustomercode()
	{
		//validate input
		$this->form_validation->set_rules('code', 'Customer code', 'required');
		...
		if ($this->form_validation->run()) {
			...
			if (!$customer) {
				$this->session->set_flashdata('msg', 'No record found');
				return redirect('customersetup/addinvoicebycode');
			}
			else {

				$id = $customer->customerid;
				return redirect(base_url()."customersetup/singlecustomerinvoice/".$id);
			}
		}
		else {
			//load view
	...
	{
		//fetch all from invoice table
		$allcustomer = $this->customersetup_model->getcustomerinvoicetable();

		$month = '';

		$customer = $allcustomer['rows'];

		foreach ($customer as $row) {
			$id = $row->customerid;

			if ($row->invoicemonth == 'January') {
				$month = '01';
			}
			elseif ($row->invoicemonth == 'February') {
				$month = '02';
			}
			elseif ($row->invoicemonth == 'March') {
				$month = '03';
			}
			elseif ($row->invoicemonth == 'April') {
				$month = '04';
			}
			elseif ($row->invoicemonth == 'May') {
				$month = '05';
			}
			elseif ($row->invoicemonth == 'June') {
				$month = '06';
			}
			elseif ($row->invoicemonth == 'July') {
				$month = '07';
			}
			elseif ($row->invoicemonth == 'August') {
				$month = '08';
			}
			elseif ($row->invoicemonth == 'September') {
				$month = '09';
			}
			elseif ($row->invoicemonth == 'October') {
				$month = '10';
			}
			elseif ($row->invoicemonth == 'November') {
				$month = '11';
			}
			else {
				$month = '12';
			...
			$updcustomer = $this->customersetup_model->updateInvoiceRecord($id, $month);

			if (!$updcustomer) {
				return redirect('customersetup/status');
			}
		}
	...
	public function viewcommercialtypes()
	{
		//select all commercial types
		$allcommercialtypes = $this->customersetup_model->getCommercialTypes();
		$types = '';
		$num = 0;
		if ($allcommercialtypes) {
			$types = $allcommercialtypes['rows'];
			$num = $allcommercialtypes['num'];
		}
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('commercial/viewcommercialtypes_view', ['types' => $types, 'num' => $num]);
		$this->load->view('inc2/footer_view5');
	}

	//----------------------------------------

	public function editcommercialtype()
	{
		if ($this->session->userdata('role') != 'Admin') {
			$this->session->set_flashdata('msg', 'Access Denied');
			return redirect('customersetup/viewcommercialtypes');
		}
		else {
		
			if ($this->input->post('typeId')) {
				$data = array(); //$x = '';
				foreach ($_POST['typeId'] as $key => $value) {
					$data['typeId'] = $_POST['typeId'][$key];
					$data['status'] = $_POST['status'][$key];
					$data['typeName'] = $_POST['typeName'][$key];

					//update staff status
					$updatetype = $this->customersetup_model->updateCommercialType($data);
				}
				$this->session->set_flashdata('success', 'Commercial type updated');
				return redirect('customersetup/viewcommercialtypes');
			}
			else {
	...
	{
		//get sector areas
		$allareas = $this->customersetup_model->getAreas();
		if (!$allareas) {
			$this->session->set_flashdata('msg', 'No record found.');
			return redirect('home/index');
		}
		...
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('reports/debtanalysis_view', ['areas' => $areas]);
		$this->load->view('inc2/footer_view5');
	}

	//--------------------------------------------

	public function fetchareasectors()
	{
		//validate input
		$this->form_validation->set_rules('areas', 'Sector Area', 'required');

		if ($this->form_validation->run()) {
			$area = $this->input->post('areas');
			$allcustomers = $this->customersetup_model->getCustomersInArea($area);
			if (!$allcustomers) {
				$this->session->set_flashdata('msg', 'No record found.');
				return redirect('customersetup/debtanalysis');
			}

			$customers = $allcustomers['rows'];

			//get all sector in area
			$allsectors = $this->customersetup_model->getSectorsInArea($area);
			if (!$allsectors) {
				$this->session->set_flashdata('msg', 'No record found.');
				return redirect('customersetup/debtanalysis');
			}
			...
			//get area name
			$areaid = $area;
			$singlearea = $this->customersetup_model->getsinglearea($areaid);
			$areaname = $singlearea->areaname;

			$this->load->view('inc/header_view');
			$this->load->view('inc/sidebar_view');
			$this->load->view('reports/sectordebt_view', ['sectordebt' => $sectordebt, 'areas' => $areas, 'areaname' => $areaname]);
			$this->load->view('inc2/footer_view6');
		}
		else {
			//get sector areas
			$allareas = $this->customersetup_model->getAreas();
			...
			$this->load->view('inc/header_view');
			$this->load->view('inc/sidebar_view');
			$this->load->view('reports/deptanalysis_view', ['areas' => $areas]);
			$this->load->view('inc2/footer_view5');
		}
	}
	...
	public function sectorDebtPerStreet()
	{
		$sector = $this->input->get('sector');
		$area = $this->input->get('area');

		//get customers in sector
		$allcustomers = $this->customersetup_model->getCustomersInSector($area, $sector);
		...
		$allstreets = $this->customersetup_model->getStreetsInSector($sector);
		if (!$allstreets) {
			$this->session->set_flashdata('msg', 'No record found.');
			return redirect('customersetup/debtanalysis');
		}
		...
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('reports/streetdebt_view', ['streetdebt' => $streetdebt, 'sector' => $sector]);
		$this->load->view('inc2/footer_view7');
	}

	//--------------------------------------------

	public function streetDebtPerHouse()
	{
		$street = $this->input->get('street');
		$sector = $this->input->get('sector');

		//get customers in street
		$allcustomers = $this->customersetup_model->getCustomersInStreet($sector, $street);
		...
		$allhouses = $this->customersetup_model->getHousesInStreet($sector, $street);
		if (!$allhouses) {
			$this->session->set_flashdata('msg', 'No record found.');
			return redirect('customersetup/debtanalysis');
		}
		...
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('reports/housedebt_view', ['housedebt' => $housedebt, 'street' => $street, 'sector' => $sector]);
		$this->load->view('inc2/footer_view7');
	}

	//-----------------------------------------

	public function customerDebtPerHouse()
	{
		...
		$allcustomers = $this->customersetup_model->getCustomersInHouse($house, $street, $sector);
		...
		$this->load->view('inc/header_view');
		$this->load->view('inc/sidebar_view');
		$this->load->view('reports/customerdebtinhouse_view', ['customers' => $customers]);
		$this->load->view('inc2/footer_view7');
	}
	...
	public function updateCustomerStatus()
	{
		...
		foreach ($invoices as $invoice) {
			foreach ($customers as $row) {
				if ($invoice->customerid == $row->customerid) {
					$status = $this->customersetup_model->updateInvoiceStatus($row);
				}
			}
		}
